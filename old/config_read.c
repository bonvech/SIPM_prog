...
//int cr=0;
int data=0;
int det=0,thl=0,thh=0,off=0,len=0,smo=0;
//int thm=0;
int m[12];
int matrix=0;
...
//		    fscanf(hf,"%i%i%i%i%i%i%i",&det,&thl,&thh,&off,&len,&smo,&thm);
		    fscanf(hf,"%i%i%i%i%i%i",&det,&thl,&thh,&off,&len,&smo);
...
//	fprintf(ffstat,"      TH_L   TH_H   Offset   Lengt   Smooth   TH_M   Matrix   Matrix_Mon\n");
	printf("      TH_L   TH_H   Offset   Lengt   Smooth   Matrix\n");
	fprintf(ffstat,"      TH_L   TH_H   Offset   Lengt   Smooth   Matrix\n");
	for(int jj=0;jj<12;jj++) {
//		printf("%3i%7i%7i%9i%8i%8i%8i    0x%X    0x%X\n",jj,
		printf("%3i%7i%7i%9i%8i%8i    0x%X\n",jj,
		    SIPM[ii].TH_Low[jj], SIPM[ii].TH_High[jj],
		    SIPM[ii].Offset[jj], SIPM[ii].Length[jj],
		    SIPM[ii].Smooth[jj], SIPM[ii].Matrix[jj]);
//		    SIPM[ii].Smooth[jj], SIPM[ii].TH_Mon[jj],
//		    SIPM[ii].Matrix[jj],SIPM[ii].Matrix_Mon[jj]);
		fprintf(ffstat,"%3i%7i%7i%9i%8i%8i    0x%X\n",jj,
