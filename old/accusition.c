...
//		    SIPM[ih].TriggerEnable_int();
	...
	this_moment=Now.time;
	last_moment=Now.time;
	printf("Start accusition data at:   "); printf("%s",(char*)ctime(&Now.time));
...
//		    SIPM[ih].TriggerDisable_int();
		...
		ftime(&Now);
	        this_moment=Now.time;
	printf("Trigger Disable at:   "); printf("%s",(char*)ctime(&Now.time));
...
//--------------------------------
		...
		ftime(&Now);
	        this_moment=Now.time;
	printf("Stop Socket at:   "); printf("%s",(char*)ctime(&Now.time));
...
//    if (fferr!=NULL) fclose(fferr);
    ...
    ftime(&Now);   // Get Time
    printf("Stop at - "); printf("%s",(char*)ctime(&Now.time));
...
//		if ( DRS[h].Type == HOST_NEW ) continue;
		...
		hht=0;mmt=0;sst=0;mlsst=0;mksst=0;dnsst=0;
