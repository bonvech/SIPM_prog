//====================================================================
/// \file acc_thread.c
/// \brief Бесконечный цикл чтения данных
/// \todo Переделать под все кластеры SIPM[i], заменить 0 на id_host
//====================================================================
/*
...
void* READ_socket_SIPM(void *number)
{
    ...
    {
        //qq(200);
        if ( SIPM[0].Stop_socket == 1 ) usleep(1000); /// спит, чтобы передавать управление другим потокам
        if ( SIPM[0].Stop_socket == 1 ) continue;

        //qq(300);
        //	sem_wait(&sem_Q);

        /// чтение 24 байта в буффер (заголовок события)
        /// \todo Добавить считвание второго пакета
        //	sz = recv(SIPM[0].sd_data, buff, 24, MSG_WAITALL );
        sz = recv(SIPM[0].sd_data, buff, 24, 0 );

        //if (sz>0) printf("sz=%i\n",sz);

        /// получены не все 24 байта, дочитать до 24 за 1 сек
        if ( (sz>0) && (sz<24) )
        {
            SumBytes=sz;
            res=0;
        ...
        /// получен весь заголовок
        if(sz == 24)
        {
            // идентификатор
            /// \todo проверку sidi. !!! Он должен быть равен 16640 (первый, длинный) или 16641 (второй, короткий).
            sidi = buff[1]*256 + buff[0];
            //printf("*********  sidi=%i\n",sidi);
            // \todo !!! если заголовок не равен 16640 или 16641 нужно искать конец пакета 0xFFFFFFFF в буфере 
            // в скользящем окне из 4 байт, начиная с buff[0]: в байтах заголовка  4,3,2,1   5,4,3,2   6,5,4,3  и т.д.
            // Если нашли конец пакета, то сразу после последнего 0xff искать новый идентификатор пакета sidi 
            // и дочитывать необходимое кол-во байт до полного заголовка.
            ...
            SIPM[0].NumEventRealy++;

            // Номер события из электроники
            ...
            /// print every 10 event to screen
            ...
            */
            ...
            /// Если не прочитаны все байты, дочитать
            if ( (szd > 0) && (szd<NumBytes) )
            {
                ...
                while (SumBytes != NumBytes)
                {
                    ...
                    if (SumBytes != NumBytes)
                    {
                        // if 1000 циклов не помогла
                        kkk++;
                        if (kkk>1000)
                        {
                            if((fferr = fopen(errname, "at")) == NULL)
            ...
            {
                // спит если не дошли данные
                usleep(1000);
            }
            /// \todo Добавить считывание второго пакета
            /*
        ...
        {
            // спит если нечего читать (нет заголовков)
            usleep(1000);
        }
        //sem_post(&sem_Q);
    } // while
